...
fn traverse_and_convert_discriminator(item: &mut Value) -> IndexMap<Value, Value> {
    if let Some(ref mut enum_variants) = get_enum(item) {
        // Remove the: ["required"][enum_discriminator] and the ["properties"][enum_discriminator]
        for (value, enum_property) in enum_variants.values_mut() {
            // Required
            if let Some(Value::Sequence(required_sequence)) = value.get_mut("required") {
                if let Some(index) =
                    required_sequence
                        .iter()
                        .enumerate()
            ...
            let can_delete_required = value.get("required").map_or(false, |required| {
        ...
        let mut additions_map = enum_variants
            .iter()
            .map(|(name, (value, _))| (Value::String(name.to_string()), (*value).clone()))
            .collect::<IndexMap<Value, Value>>();

        // Visit the children of the value type to see if any of them are applicable
        let child_addition_maps = match item {
            ...
            Value::Mapping(mapping) => mapping
                ...
                .map(traverse_and_convert_discriminator)
                .collect::<Vec<IndexMap<Value, Value>>>(),
            Value::Tagged(tagged) => vec![traverse_and_convert_discriminator(&mut tagged.value)],
                    ...
                    mapping.insert(
                        ...
                        Value::Mapping({
                            ...
                            for enum_discriminator in enum_discriminators {
                                let key = Value::String(enum_discriminator.to_string());
...
fn get_enum(item: &mut Value) -> Option<IndexMap<String, (&mut Value, String)>> {
