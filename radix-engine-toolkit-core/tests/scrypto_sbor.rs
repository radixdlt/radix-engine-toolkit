...
fn scrypto_value_can_be_encoded() {
    ...
    // Act
    let result = radix_engine_toolkit_core::functions::scrypto_sbor::encode(&value);
...
fn scrypto_value_can_be_encoded_and_decoded_later() {
    // Arrange
    let value = ScryptoValue::Bool { value: false };
    let encoded = radix_engine_toolkit_core::functions::scrypto_sbor::encode(&value).unwrap();

    // Act
    let decoded = radix_engine_toolkit_core::functions::scrypto_sbor::decode(encoded);
