...
pub fn modify(
    ...
    // Adding the lock fee instruction. This depends if the first instruction in the manifest is a
    // call to withdraw from an account or not. If yes, then we transform the call to `withdraw` to
    // a `lock_fee_and_withdraw`. Same thing happens for the non-fungible variant of the method. If
    // the first method is not a withdraw method, then we just insert a new instruction that locks
    // a fee against said account.
    if let Some((lock_fee_account, lock_fee_amount)) = modifications.add_lock_fee {
        match instructions.first_mut() {
            ...
            _ => {
                ...
                // 3. There is a call method instruction but it's not to the same account that we
                //    want to lock fees against.
                // 4. There is a call method instruction to the account that we want to lock fees
                //    against but it's not to the account withdraw methods.
                instructions.insert(
                    0,
                    InstructionV1::CallMethod {
                        address: DynamicGlobalAddress::Static(lock_fee_account.into()),
                        method_name: ACCOUNT_LOCK_FEE_IDENT.to_owned(),
                        args: to_manifest_value_and_unwrap!(&AccountLockFeeInput {
...
pub struct TransactionManifestModifications {
    /// The [`ComponentAddress`]es of the access controllers to add create proof instructions in
    /// the manifest for.
