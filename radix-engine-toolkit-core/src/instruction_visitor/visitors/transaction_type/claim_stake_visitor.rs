...
pub struct ClaimStakeVisitor<'r> {
    ...
    /// The finite state machine used by the visitor to determine if this transaction is an claim
    /// transaction or not.
    ...
    /// The claims made in the transaction - the non-fungible data is put as unit since they will
    /// be resolved by the visitor as finalization.
...
pub enum ClaimStakeFiniteStateMachine {
    /// The transaction has been marked as being not an claim transaction. The boolean this enum
    /// holds is whether this decision is final or not. As an example, the initial state of the FSM
    /// is [`ClaimStakeFiniteStateMachine::NotAnClaimTransaction(false)`] indicating that we have
    /// not yet concluded whether it's an claim transaction or not but we're still open to it.
    NotAnClaimTransaction(bool),

    /// The Claim NFTs have been withdrawn from an account and are currently in the worktop. This
    /// keeps track of the address of what has been withdrawn, the amount that has been withdrawn,
    /// and what account it has been withdrawn from.
    ClaimNftWithdrawn {
        /// The address of the account that the Claim NFTs were withdrawn from.
        account_withdrawn_from: ComponentAddress,
        /// The address of the resource that was withdrawn - it is checked that this is a fungible
        /// resource address.
    ...
    /// The LSUs have been taken from the worktop and put into a [`ManifestBucket`].
    ClaimNftInBucket {
        /// The address of the account that the Claim NFTs were withdrawn from.
        account_withdrawn_from: ComponentAddress,
        /// The address of the resource that was withdrawn - it is checked that this is a fungible
    ...
    /// The claim NFT has been given to the validator and it has given back XRD.
    Claimed {
        /// The address of the account that the Claim NFTs were withdrawn from.
        account_withdrawn_from: ComponentAddress,
    ...
    XrdInBucket {
        /// The address of the account that the Claim NFTs were withdrawn from - we would like to
        /// make sure that the LSUs are deposited back into the same account.
        account_withdrawn_from: ComponentAddress,
    },
...
impl ClaimStakeFiniteStateMachine {
    ...
    /// the FSM transitions into [`ClaimStakeFiniteStateMachine::NotAnClaimTransaction(true)`]
    ...
    pub fn transition(
        ...
        match (&self, instruction) {
            // We can transition to [`ClaimNftWithdrawn`] if we're in the initial state or if we are
            // in the [`DepositedIntoAccount`] state and we get a CallMethod to account withdraw.
            (
            ...
            ) if is_account(&address) && method_name == ACCOUNT_WITHDRAW_NON_FUNGIBLES_IDENT => {
                    ...
                    .and_then(|encoded| manifest_decode(&encoded).ok())
                {
                    Some(AccountWithdrawNonFungiblesInput {
                        resource_address,
                        ids,
                    }) if !resource_address.is_fungible() => (resource_address, ids),
                    _ => {
            ...
            // Transition from ClaimNftWithdrawn -> ClaimNftInBucket. We can make this transition
            // with any of the take from worktop instructions.
            (
            ...
            ) if &ids.iter().cloned().collect::<BTreeSet<_>>() == claim_nft_local_ids
                && resource_address == *claim_nft_resource =>
            {
                *self = Self::ClaimNftInBucket {
                    account_withdrawn_from: *account_withdrawn_from,
            ...
            ) if is_validator(&address) && method_name == VALIDATOR_CLAIM_XRD_IDENT => {
                let validator_address = ComponentAddress::try_from(address).expect("Must succeed");

                let Some(claimed_xrd) = get_returned_xrd_amount(execution_trace, instruction_index)
                else {
                    *self = Self::NotAnClaimTransaction(true);
                    return;
                };
            ...
            // For anything else transition into a final state of [`NotAnClaimTransaction()`]
...
fn get_returned_xrd_amount(
    ...
    {
        Some([WorktopChange::Put(ResourceSpecifier::Amount(XRD, amount))]) => Some(*amount),
        None | Some([..]) => None,
    }
}
