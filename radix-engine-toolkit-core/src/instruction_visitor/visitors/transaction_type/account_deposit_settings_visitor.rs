    ...
    fn visit_instruction(
        ...
        match instruction {
            InstructionV1::CallMethod {
                ...
                // Filter: Any method call to something that is not an account is not permitted.
                ...
                match method_name.as_str() {
                    ...
                    ACCOUNT_ADD_AUTHORIZED_DEPOSITOR => {
                        if let Some(AccountAddAuthorizedDepositorInput { badge }) =
                            to_manifest_type(args)
                        {
                            self.authorized_depositors_changes
                                .entry(component_address)
                    ...
                    ACCOUNT_REMOVE_AUTHORIZED_DEPOSITOR => {
                        if let Some(AccountRemoveAuthorizedDepositorInput { badge }) =
                            to_manifest_type(args)
                        {
                            self.authorized_depositors_changes
                                .entry(component_address)
            ...
            /* All other instructions are not allowed and put us in an illegal state */
