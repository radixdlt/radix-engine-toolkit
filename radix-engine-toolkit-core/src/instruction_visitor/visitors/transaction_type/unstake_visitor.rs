...
pub struct UnstakeVisitor<'r> {
    ...
    /// The finite state machine used by the visitor to determine if this transaction is an ustake
    /// transaction or not.
    ...
    /// The unstakes made in the transaction - the non-fungible data is put as unit since they will
    /// be resolved by the visitor as finalization.
    unstakes: Vec<UnstakeInformation<()>>,
}

impl<'r> UnstakeVisitor<'r> {
    pub fn new(transaction_receipt: &'r ExecutionAnalysisTransactionReceipt) -> Self {
        Self {
            transaction_receipt,
    ...
    pub fn output(self) -> Option<Vec<UnstakeInformation<UnstakeData>>> {
        ...
        let non_fungible_data = data_of_newly_minted_non_fungibles(self.transaction_receipt);
        self.unstakes
            .into_iter()
            .map(
                |UnstakeInformation {
...
pub enum UnstakeFiniteStateMachine {
    /// The transaction has been marked as being not an unstaking transaction. The boolean this
    /// enum holds is whether this decision is final or not. As an example, the initial state
    /// of the FSM is [`UnstakeFiniteStateMachine::NotAnUnstakeTransaction(false)`] indicating
    /// that we have not yet concluded whether it's an unstake transaction or not but we're
    /// still open to it.
    NotAnUnstakeTransaction(bool),

    /// The LSUs have been withdrawn from an account and are currently in the worktop. This keeps
    /// track of the address of what has been withdrawn, the amount that has been withdrawn, and
    /// what account it has been withdrawn from.
    ...
    /// The LSUs have been taken from the worktop and put into a [`ManifestBucket`]. This state is
    ...
    /// LSUs have been sent to a validator for unstaking and the validator has returned a claim NFT
    /// back.
    Unstaked {
        /// The address of the account that the XRD was withdrawn from.
        account_withdrawn_from: ComponentAddress,
...
impl UnstakeFiniteStateMachine {
    ...
    pub fn transition(
        ...
        match (&self, instruction) {
            // We can transition to [`LiquidStakeUnitsWithdrawn`] if we're in the initial state or
            // if we are in the [`DepositedIntoAccount`] state and we get a CallMethod to account
            // withdraw.
            (
                Self::NotAnUnstakeTransaction(false) | Self::DepositedIntoAccount,
                    ...
                    .and_then(|encoded| manifest_decode(&encoded).ok())
                {
                    Some(AccountWithdrawInput {
                        resource_address,
                        amount,
                    }) if resource_address.is_fungible() => (resource_address, amount),
                    _ => {
            ...
            // Transition from LiquidStakeUnitsWithdrawn -> LiquidStakeUnitsInBucket. This happens
            // when we get a TakeFromWorktop or a TakeAllFromWorktop
            (
            ...
            ) if amount == *stake_unit_amount && resource_address == *stake_unit_address => {
            ...
            ) if is_validator(&address) && method_name == VALIDATOR_UNSTAKE_IDENT => {
            ...
            ) if resource_address == *claim_nft_resource && amount == Decimal::ONE => {
            ...
            // For anything else transition into a final state of [`NotAnUnstakeTransaction()`]
...
fn get_returned_claim_nft(
    ...
    {
        Some([WorktopChange::Put(ResourceSpecifier::Ids(resource_address, ids))]) => ids
            .iter()
            .next()
