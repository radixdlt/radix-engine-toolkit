...
impl Instruction {
    pub fn from_native(native: &NativeInstruction, network_id: u8) -> Self {
        match native {
            ...
            NativeInstruction::ReturnToWorktop { bucket_id } => {
            ...
            NativeInstruction::PushToAuthZone { proof_id } => {
                Self::PushToAuthZone {
            ...
            NativeInstruction::DropAuthZoneRegularProofs => {
                Self::DropAuthZoneRegularProofs
            }
            NativeInstruction::DropAuthZoneSignatureProofs => {
            ...
            NativeInstruction::BurnResource { bucket_id } => {
                Self::BurnResource {
                    bucket_id: (*bucket_id).into(),
                }
            }
            ...
            NativeInstruction::CallDirectVaultMethod {
                ...
                address: Arc::new(
                    Address::from_typed_node_id(*address, network_id)
                ),
                method_name: method_name.to_owned(),
                args: ManifestValue::from_native(args, network_id),
            },
    ...
    pub fn to_native(&self) -> Result<NativeInstruction> {
        let value = match self {
            ...
            Self::ReturnToWorktop { bucket_id } => {
                NativeInstruction::ReturnToWorktop {
                    bucket_id: (*bucket_id).into(),
                }
            }
            Self::AssertWorktopContains {
            ...
            Self::PushToAuthZone { proof_id } => {
                NativeInstruction::PushToAuthZone {
                    proof_id: (*proof_id).into(),
                }
            }
            ...
            Self::BurnResource { bucket_id } => {
                NativeInstruction::BurnResource {
                    bucket_id: (*bucket_id).into(),
                }
            }
