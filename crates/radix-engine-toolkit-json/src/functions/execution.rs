...
use radix_engine_toolkit::instruction_visitor::visitors::transaction_type::account_deposit_settings_visitor::AuthorizedDepositorsChanges;
use radix_engine_toolkit::instruction_visitor::visitors::transaction_type::account_deposit_settings_visitor::ResourcePreferenceAction;
use radix_engine_toolkit::instruction_visitor::visitors::transaction_type::reserved_instructions::ReservedInstruction;
use radix_engine_toolkit::instruction_visitor::visitors::transaction_type::transfer_visitor::*;
use radix_engine_toolkit::instruction_visitor::visitors::transaction_type::general_transaction_visitor::*;
use schemars::*;
...
impl<'f> Function<'f> for ExecutionAnalyze {
    ...
    fn handle(
        ...
        let receipt =
            scrypto_decode::<VersionedTransactionReceipt>(&preview_receipt)
                .map_err(|error| {
        ...
        let execution_analysis =
            ExecutionAnalysisTransactionReceipt::new(&receipt)
                .and_then(|receipt| {
                    radix_engine_toolkit::functions::execution::analyze(
                        &instructions,
                        &receipt,
                    )
                })
                .map_err(|error| {
                    InvocationHandlingError::ExecutionModuleError(debug_string(
                        error,
                    ))
                })?;
...
pub enum SerializableTransactionType {
    ...
    AccountDepositSettings(
        Box<SerializableAccountDepositSettingsTransactionType>,
    ),
...
impl From<radix_engine_toolkit::functions::execution::FeeLocks>
    for SerializableFeeLocks
{
    fn from(
        value: radix_engine_toolkit::functions::execution::FeeLocks,
    ) -> Self {
        Self {
...
pub struct SerializableTransferTransactionType {
    pub from: SerializableNodeId,
    pub transfers: HashMap<
        SerializableNodeId,
        HashMap<SerializableNodeId, SerializableResources>,
    >,
}
...
pub struct SerializableAccountDepositSettingsTransactionType {
    ...
    pub default_deposit_rule_changes:
        HashMap<SerializableNodeId, SerializableDefaultDepositRule>,
...
pub struct SerializableGeneralTransactionType {
    ...
    pub account_withdraws:
        HashMap<SerializableNodeId, Vec<SerializableResourceTracker>>,
    pub account_deposits:
        HashMap<SerializableNodeId, Vec<SerializableResourceTracker>>,
    pub addresses_in_manifest: InstructionsExtractAddressesOutput,
    pub metadata_of_newly_created_entities: HashMap<
        SerializableNodeId,
        HashMap<String, Option<SerializableMetadataValue>>,
    >,
    pub data_of_newly_minted_non_fungibles: HashMap<
        SerializableNodeId,
        HashMap<SerializableNonFungibleLocalId, SerializableBytes>,
    >,
}
    ...
    pub fn new(value: AuthorizedDepositorsChanges, network_id: u8) -> Self {
        Self {
            added: value
                ...
                .into_iter()
                .map(|value| {
                    SerializableResourceOrNonFungible::new(value, network_id)
                })
                .collect(),
            removed: value
                .removed
    ...
    pub fn new(value: ResourceOrNonFungible, network_id: u8) -> Self {
        match value {
            ResourceOrNonFungible::Resource(resource_address) => {
                Self::Resource(SerializableNodeId::new(
                    resource_address.into_node_id(),
                    network_id,
                ))
            }
            ResourceOrNonFungible::NonFungible(non_fungible) => {
                Self::NonFungible(SerializableNonFungibleGlobalId::new(
                    non_fungible,
                    network_id,
                ))
            }
...
impl From<SerializableResourcePreferenceAction> for ResourcePreferenceAction {
    fn from(value: SerializableResourcePreferenceAction) -> Self {
        match value {
            ...
            SerializableResourcePreferenceAction::Set(value) => {
                Self::Set(value.into())
            }
        }
    }
...
impl From<SerializableDefaultDepositRule> for DefaultDepositRule {
    fn from(value: SerializableDefaultDepositRule) -> Self {
        match value {
            SerializableDefaultDepositRule::Accept => {
                DefaultDepositRule::Accept
            }
            SerializableDefaultDepositRule::Reject => {
                DefaultDepositRule::Reject
            }
            SerializableDefaultDepositRule::AllowExisting => {
    ...
    pub fn new(metadata: MetadataValue, network_id: u8) -> Self {
        match metadata {
            MetadataValue::String(value) => {
                SerializableMetadataValue::String(value)
            }
            MetadataValue::Bool(value) => {
                SerializableMetadataValue::Bool(value)
            }
            MetadataValue::U8(value) => {
                SerializableMetadataValue::U8(value.into())
            }
            MetadataValue::U32(value) => {
                SerializableMetadataValue::U32(value.into())
            }
            MetadataValue::U64(value) => {
                SerializableMetadataValue::U64(value.into())
            }
            MetadataValue::I32(value) => {
                SerializableMetadataValue::I32(value.into())
            }
            MetadataValue::I64(value) => {
                SerializableMetadataValue::I64(value.into())
            }
            MetadataValue::Decimal(value) => {
                SerializableMetadataValue::Decimal(value.into())
            }
            MetadataValue::GlobalAddress(value) => {
                SerializableMetadataValue::GlobalAddress(
                    SerializableNodeId::new(value.into_node_id(), network_id),
                )
            }
            MetadataValue::PublicKey(value) => {
                SerializableMetadataValue::PublicKey(value.into())
            }
            ...
            MetadataValue::Instant(value) => {
                SerializableMetadataValue::Instant(
                    value.seconds_since_unix_epoch.into(),
                )
            }
            MetadataValue::Url(value) => {
                SerializableMetadataValue::Url(value.0)
            }
            MetadataValue::Origin(value) => {
                SerializableMetadataValue::Origin(value.0)
            }
            ...
            MetadataValue::StringArray(value) => {
                SerializableMetadataValue::StringArray(value)
            }
            MetadataValue::BoolArray(value) => {
                SerializableMetadataValue::BoolArray(value)
            }
            MetadataValue::U8Array(value) => {
                SerializableMetadataValue::U8Array(array_into!(value))
            }
            ...
            MetadataValue::NonFungibleGlobalIdArray(value) => {
                SerializableMetadataValue::NonFungibleGlobalIdArray(
                    value
                        .into_iter()
                        .map(|id| {
                            SerializableNonFungibleGlobalId::new(id, network_id)
                        })
                        .collect(),
                )
            }
            MetadataValue::NonFungibleLocalIdArray(value) => {
                SerializableMetadataValue::NonFungibleLocalIdArray(array_into!(
                    value
                ))
            }
            MetadataValue::InstantArray(value) => {
                SerializableMetadataValue::InstantArray(
                    value
                        .into_iter()
            ...
            MetadataValue::UrlArray(value) => {
                SerializableMetadataValue::UrlArray(
                    value.into_iter().map(|value| value.0).collect(),
                )
            }
            MetadataValue::OriginArray(value) => {
                SerializableMetadataValue::OriginArray(
                    value.into_iter().map(|value| value.0).collect(),
                )
            }
            MetadataValue::PublicKeyHashArray(value) => {
                SerializableMetadataValue::PublicKeyHashArray(array_into!(
                    value
                ))
            }
        }
    ...
    pub fn new(resource_tracker: ResourceTracker, network_id: u8) -> Self {
        match resource_tracker {
            ...
            ResourceTracker::NonFungible {
                ...
                amount: SerializableSource::new(amount, Into::into),
                ids: SerializableSource::new(ids, |ids| {
...
impl From<SerializableReservedInstruction> for ReservedInstruction {
    fn from(value: SerializableReservedInstruction) -> Self {
        match value {
            SerializableReservedInstruction::AccessController => {
                Self::AccessController
            }
            SerializableReservedInstruction::AccountLockFee => {
                Self::AccountLockFee
            }
            SerializableReservedInstruction::AccountSecurify => {
                Self::AccountSecurify
            }
            SerializableReservedInstruction::IdentitySecurify => {
                Self::IdentitySecurify
            }
            SerializableReservedInstruction::AccountUpdateSettings => {
