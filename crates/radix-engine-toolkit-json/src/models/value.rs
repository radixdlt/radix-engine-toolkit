...
use radix_transactions::data::{
    from_decimal, from_non_fungible_local_id, from_precise_decimal, to_decimal,
    to_non_fungible_local_id, to_precise_decimal,
};
...
impl From<SerializableManifestValueKind> for ManifestValueKind {
    fn from(value: SerializableManifestValueKind) -> Self {
        match value {
            ...
            SerializableManifestValueKind::Bucket => {
                Self::Custom(ManifestCustomValueKind::Bucket)
            }
            SerializableManifestValueKind::Proof => {
                Self::Custom(ManifestCustomValueKind::Proof)
            }
            SerializableManifestValueKind::Expression => {
                Self::Custom(ManifestCustomValueKind::Expression)
            }
            SerializableManifestValueKind::Blob => {
                Self::Custom(ManifestCustomValueKind::Blob)
            }
        ...
        let value = match manifest_value {
            ...
            ManifestValue::Map {
                ...
                entries: entries
                    .iter()
                    .map(|(key, value)| {
                        Self::from_manifest_value(key, network_id).and_then(
                            |key| {
                                Self::from_manifest_value(value, network_id)
                                    .map(|value| SerializableMapEntry {
                                        key,
                                        value,
                                    })
                            },
    ...
    pub fn to_manifest_value(
        ...
        let value = match self {
            ...
            Self::Map {
                ...
                entries: entries
                    ...
                    .map(|SerializableMapEntry { key, value }| {
                        key.to_manifest_value().and_then(|key| {
                            value.to_manifest_value().map(|value| (key, value))
                        })
                    })
                    .collect::<Result<_, _>>()?,
            },
            ...
            Self::PreciseDecimal { value } => ManifestValue::Custom {
                value: ManifestCustomValue::PreciseDecimal(
                ...
                SerializableManifestAddress::Static(value) => {
                    ManifestValue::Custom {
                        value: ManifestCustomValue::Address(
                            ManifestAddress::Static(value.0.node_id),
                        ),
                    }
                }
