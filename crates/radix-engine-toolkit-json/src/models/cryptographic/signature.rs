...
impl From<SignatureV1> for SerializableSignature {
    fn from(value: SignatureV1) -> Self {
        match value {
            SignatureV1::Secp256k1(signature) => {
                Self::Secp256k1(signature.0.into())
            }
            SignatureV1::Ed25519(signature) => {
                Self::Ed25519(signature.0.into())
            }
        }
    }
}
...
impl From<SerializableSignature> for SignatureV1 {
    fn from(value: SerializableSignature) -> Self {
        match value {
            ...
            SerializableSignature::Ed25519(value) => {
                Self::Ed25519(Ed25519Signature(*value))
            }
        }
    }
}
