...
fn intent_can_be_compiled_and_later_decompiled() {
    ...
    let intent = test_data::intent();
    let compiled =
        radix_engine_toolkit_core::functions::intent::compile(&intent).unwrap();

    // Act
    let decompiled =
        radix_engine_toolkit_core::functions::intent::decompile(compiled);
...
fn intent_can_be_statically_validated() {
    ...
    let validation_result =
        radix_engine_toolkit_core::functions::intent::statically_validate(
            &intent,
            validation_config,
        );
