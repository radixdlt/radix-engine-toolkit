...
impl SimpleTransactionTypeVisitor {
    pub fn output(
        self,
    ) -> Option<(ComponentAddress, ComponentAddress, ResourceSpecifier)> {
...
impl InstructionVisitor for SimpleTransactionTypeVisitor {
    ...
    fn visit_call_method(
        ...
        if is_account(address) {
            ...
            ) = (method_name, to_manifest_type(args))
            {
                ...
                method_name,
                (
                    validate_manifest_value_against_schema::<AccountDepositInput>(
                        args,
                    ),
    ...
    #[inline]
    fn visit_drop_auth_zone_proofs(
        &mut self,
    ) -> Result<(), InstructionVisitorError> {
        self.illegal_instruction_encountered = true;
        Ok(())
    ...
    #[inline]
    fn visit_drop_auth_zone_signature_proofs(
        &mut self,
    ) -> Result<(), InstructionVisitorError> {
        self.illegal_instruction_encountered = true;
        Ok(())
...
impl StateMachine {
    ...
    pub fn transition_to_take_from_worktop(
        &mut self,
    ) -> Result<(), StateMachineError> {
        match self {
            ...
            Self::None | Self::TakeFromWorktop(..) | Self::Deposit(..) => {
    ...
    pub fn transition_to_deposit(
        ...
        match self {
            Self::TakeFromWorktop(from_account, resources) => {
                *self =
                    Self::Deposit(*from_account, to_account, resources.clone());
                Ok(())
            }
            Self::None | Self::Deposit(..) | Self::AccountWithdraw(..) => {
                Err(StateMachineError)
            }
        }
    }
    ...
    pub fn output(
        ...
        (ComponentAddress, ComponentAddress, ResourceSpecifier),
        StateMachineError,
    > {
        match self {
            ...
            Self::None
            | Self::TakeFromWorktop(..)
            | Self::AccountWithdraw(..) => Err(StateMachineError),
        }
    }
}
