...
#[test]
fn subintent_manifest_of_transfer_is_a_general_subintent() {
    let account =
...
fn subintent_manifest_of_transfer_with_metadata_update_is_not_a_general_subintent()
 {
    ...
    subintent_manifest_classification_test(
        ManifestBuilder::new_subintent_v2()
            ...
            .take_all_from_worktop(XRD, "bucket")
            .try_deposit_or_abort(account, None, "bucket")
            .set_metadata(account, "some_key", "some_value")
            .yield_to_parent(())
            .build(),
        false,
    )
...
fn subintent_manifest_classification_test(
    subintent_manifest: SubintentManifestV2,
    is_general_subintent: bool,
) {
    let static_analysis = statically_analyze_and_validate(&subintent_manifest)
        .expect("Static analysis failed");
    let contains_general_subintent_classification = static_analysis
        .classification
        .contains(&ManifestClass::GeneralSubintent);
    if is_general_subintent {
