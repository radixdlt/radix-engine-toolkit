...
use radix_engine_toolkit::instruction_visitor::visitors::account_interactions_visitor::AccountInteractionsVisitor;
use radix_engine_toolkit::instruction_visitor::visitors::identity_interactions_visitor::IdentityInteractionsVisitor;
...
fn identity_set_component_royalty_interactions_count_as_auth_requiring_interactions(
    ...
    // Act
    let identities_requiring_auth =
        identity_interactions(&manifest.instructions);
...
fn account_interactions(
    ...
    let mut account_interactions_visitor =
        AccountInteractionsVisitor::default();
    traverse(instructions, &mut [&mut account_interactions_visitor]).unwrap();
    account_interactions_visitor.output()
}

fn identity_interactions(
    instructions: &[InstructionV1],
) -> HashSet<ComponentAddress> {
    let mut identity_interactions_visitor =
        IdentityInteractionsVisitor::default();
