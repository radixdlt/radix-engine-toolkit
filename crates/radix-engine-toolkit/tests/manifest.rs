...
fn manifest_hash_can_be_obtained() {
    ...
    // Act
    let manifest_hash =
        radix_engine_toolkit::functions::manifest::hash(&manifest);
...
fn manifest_can_be_compiled_and_later_decompiled() {
    ...
    let manifest = test_data::manifest();
    let compiled =
        radix_engine_toolkit::functions::manifest::compile(&manifest).unwrap();
...
fn manifest_can_be_statically_validated() {
    ...
    let validation_result =
        radix_engine_toolkit::functions::manifest::statically_validate(
            &manifest,
        );
...
fn assertion_out_of_bounds_results_in_an_error_not_a_panic() {
    ...
    // Act
    let result = radix_engine_toolkit::functions::manifest::modify(
        &manifest,
        modifications,
    );
...
fn adding_lock_fee_with_no_existing_call_to_withdraw_inserts_a_new_instruction_even_when_manifest_is_empty(
    ...
    // Assert
    let expected_instructions = vec![InstructionV1::CallMethod(CallMethod {
        address: DynamicGlobalAddress::Static(account(1).into()),
        method_name: ACCOUNT_LOCK_FEE_IDENT.to_owned(),
        args: to_manifest_value_and_unwrap!(&AccountLockFeeInput {
            amount: dec!("10")
