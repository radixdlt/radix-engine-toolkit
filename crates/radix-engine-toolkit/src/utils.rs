...
pub fn metadata_of_newly_created_entities(
    ...
    InvalidEntityTypeIdError,
> {
    let addresses = addresses_of_newly_created_entities(receipt)?;
    let mut map =
        HashMap::<GlobalAddress, HashMap<String, Option<MetadataValue>>>::new();
            ...
            .get(global_address.as_node_id())
        {
            ...
            for (substate_key, data) in entries.into_iter() {
                ...
                .and_then(|typed_substate_key| {
                    ...
                    let TypedMetadataModuleSubstateKey::MetadataEntryKey(key) =
                        key;
...
pub fn data_of_newly_minted_non_fungibles(
    ...
    let mut map = HashMap::<
        ResourceAddress,
        HashMap<NonFungibleLocalId, ScryptoValue>,
    >::new();
    let commit_result = receipt.expect_commit_success();

    for (node_id, node_state_update) in
        commit_result.state_updates.by_node.iter()
    {
        ...
        let entries: IndexMap<(PartitionNumber, SubstateKey), Vec<u8>> =
            by_partition
                .iter()
                .flat_map(|(partition_number, partition_state_update)| {
                    match partition_state_update {
                        PartitionStateUpdates::Delta { by_substate } => {
                            by_substate
                                .iter()
                                .filter_map(|(key, value)| match value {
                                    DatabaseUpdate::Set(value) => Some((
                                        (*partition_number, key.clone()),
                                        value.clone(),
                                    )),
                                    DatabaseUpdate::Delete => None,
                                })
                                .collect::<IndexMap<
                                    (PartitionNumber, SubstateKey),
                                    Vec<u8>,
                                >>()
                        }
                        PartitionStateUpdates::Batch(
                            ...
                            .map(|(substate_key, data)| {
                                (
                                    (*partition_number, substate_key.clone()),
                                    data.clone(),
                                )
                            })
                            .collect(),
