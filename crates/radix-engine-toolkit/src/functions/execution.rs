...
pub fn analyze(
    ...
    let mut account_deposit_settings_visitor =
        AccountDepositSettingsVisitor::default();
    let mut general_transaction_visitor =
        GeneralTransactionTypeVisitor::new(execution_trace);
    let mut reserved_instructions_visitor =
        ReservedInstructionsVisitor::default();
    ...
    {
        transaction_types.push(TransactionType::AccountDepositSettings(
            Box::new(AccountDepositSettingsTransactionType {
                resource_preference_changes,
                default_deposit_rule_changes,
                authorized_depositors_changes,
            }),
    ...
    if let Some(claim_stake) = claim_stake_visitor.output() {
        transaction_types.push(TransactionType::ClaimStakeTransaction(
            Box::new(ClaimStakeTransactionType(claim_stake)),
        ))
    }
    if let Some((account_withdraws, account_deposits)) =
        general_transaction_visitor.output()
    {
    ...
    let fee_summary = FeeSummary {
        ...
        finalization_cost: preview_receipt
            .fee_summary
            .total_finalization_cost_in_xrd,
        storage_expansion_cost: preview_receipt
            .fee_summary
...
/// A transaction receipt used for execution analysis. This struct maintains the
/// invariant that the execution of the transaction succeeded and was committed
/// to ledger state and that there is an execution trace output.
...
impl<'r> AsRef<TransactionReceiptV1>
    for ExecutionAnalysisTransactionReceipt<'r>
{
