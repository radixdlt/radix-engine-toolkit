...
impl SchemaVisitor<ScryptoCustomSchema> for BucketInPathVisitor {
    ...
    fn visit_custom(
        ...
        custom_type_kind: &<ScryptoCustomSchema as sbor::CustomSchema>::CustomLocalTypeKind<
        >,
    ) -> Result<(), Self::Error> {
        let type_validation =
            schema.resolve_type_validation(local_type_id).map_or(
...
impl BucketInPathVisitor {
    pub fn path_contains_bucket(self) -> bool {
        self.0
    }
}
