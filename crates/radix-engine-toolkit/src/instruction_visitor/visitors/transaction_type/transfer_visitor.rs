...
pub struct TransferTransactionTypeVisitor {
    ...
    // Controls whether lock fee instructions are allowed or not - defaults to
    // false.
        ...
        match instruction {
            ...
            InstructionV1::CallMethod {
                ...
                match method_name.as_str() {
                    ...
                    | ACCOUNT_TRY_DEPOSIT_BATCH_OR_ABORT_IDENT => self
                        .handle_validation_and_account_deposits(
                            component_address,
                            args,
                        )?,
            ...
            InstructionV1::ReturnToWorktop { bucket_id } => {
                let (resource_address, resources) = self
                    .bucket_tracker
                    .remove(bucket_id)
                    .ok_or(TransferTransactionTypeError::BucketNotFound(
                        *bucket_id,
                    ))?;
                self.worktop
...
impl TransferTransactionTypeVisitor {
    fn handle_account_withdraw(
        ...
        if let Some(account_withdrawn_from_in_the_past) =
    ...
    fn handle_validation_and_account_deposits(
        ...
        // Validate that this follows either the deposit or deposit bach schema.
        // If it does, then extract all buckets and expressions and deal
        // with them.

        let matches_deposit_schema = [
            validate_manifest_value_against_schema::<AccountDepositInput>(args),
            validate_manifest_value_against_schema::<AccountDepositBatchInput>(
                args,
            ),
            validate_manifest_value_against_schema::<
        ...
        for (resource_address, resources) in resources_to_deposit {
            let deposits_map =
                self.account_deposits.entry(component_address).or_default();
            if let Some(deposited_resources) =
                deposits_map.get_mut(&resource_address)
            {
...
impl Worktop {
    ...
    fn take(
        ...
        let worktop_contents = self.worktop.get_mut(&resource_address).ok_or(
            WorktopError::ContentValidationError(
                resource_address,
    ...
    fn drain(
        &mut self,
    ) -> std::collections::hash_map::Drain<ResourceAddress, Resources> {
        self.worktop.drain()
    }
}
...
impl Resources {
    ...
    fn checked_sub_ids(
        &self,
        other: &IndexSet<NonFungibleLocalId>,
    ) -> Option<Self> {
        match self {
            Self::Ids(ids) => {
