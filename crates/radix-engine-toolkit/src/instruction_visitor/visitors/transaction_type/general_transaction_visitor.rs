...
// =================================================================================================
// Note: There is a chance that the receipt that has been passed doesn't belong
// to the same manifest or transaction. Therefore, we do not do unwraps or
// panics in this module at all. As an example, in this module, it's totally
// normal to come across a `TakeFromWorktop` instruction that has
// worktop changes of `Worktop::Put` because the receipt is actually of a
// totally different manifest that does different things. When we encounter such
// cases we simply return an error letting the caller know of the fact that
// there is a mismatch. We do not need to be too clear about what the
// mismatch is.
// =================================================================================================
...
impl<'r> GeneralTransactionTypeVisitor<'r> {
    ...
    pub fn handle_call_method(
        ...
        // transaction type allows for or not.
        if !global_address
            .as_node_id()
            .entity_type()
        ...
        if ACCOUNT_WITHDRAW_METHODS.contains(&method_name.to_string())
            && is_account(global_address)
        {
            ...
            // If some amount if withdrawn from the account but we encounter no
            // worktop changes then we MUST error out! This is
            // because a withdraw of 0 will return a bucket which must be
            // seen in the worktop changes here. Thus, the choice to return a
            // `WorktopChangesError` error instead of continuing to
            // the next instruction is deliberate here.
                ...
                if let Ok(AccountWithdrawNonFungiblesInput {
                    ...
                    match worktop_puts {
                        ...
                        ResourceSpecifier::Ids(
                            ...
                            ResourceTracker::NonFungible {
                                resource_address,
                                amount: Source::Guaranteed(set_decimal_len(
                                    &ids,
                                )),
            ...
            && is_account(global_address)
        {
            ...
            if !expressions.is_empty() {
                // There are cases when even through there is an expression such
                // as EntireWorktop we may have no worktop
                // changes and thus no work or handling to do. As an example,
                // in the case that we do a single instruction manifest that
                // does a deposit all into an account from an
                // empty worktop, there is already nothing in the worktop. So,
                // we can't assume that there will be worktop
    ...
    pub fn handle_take_from_worktop(
        ...
        // This depends on whether the resource is fungible or non-fungible. If
        // the resource is fungible, then we can just construct a
        // ResourceTracker::Fungible of the amount given
        // here as guaranteed and move on. If it's non-fungible however, then
        // the amount in here is guaranteed but the non-fungible local
        // ids are not guaranteed since we obtain them by looking at the
        // bucket snapshot from the receipt.
    ...
    pub fn handle_take_non_fungibles_from_worktop(
        ...
        // In this case, the resource is non-fungible and the take from worktop
        // is of a known set of ids, which we can also say of a known
        // set of amounts. Thus, everything about this can be
        // found statically and we can straight away map this to a resource
        // tracker without the need to look at the worktop changes.
    ...
    pub fn handle_take_all_from_worktop(
        ...
        // This case changes slightly between fungible and non-fungible
        // resources. However, all of the cases result in everything
        // being predicted and nothing being guaranteed. We observe
        // the worktop changes and then based on that construct the resource
        // tracker of whatever the observed bucket amounts/ids were.
        let resource_tracker = match self
            .execution_trace
            .worktop_changes()
            .get(&self.instruction_index)
...
/// Given an entity type and the name of a method or function on that entity,
/// this method determines if this method is permitted in general transaction or
/// not.
