...
pub struct StakeVisitor<'r> {
    ...
    /// transaction is a stake transaction or not.
...
pub enum StakeFiniteStateMachine {
    /// The transaction has been marked as being not a staking transaction. The
    /// boolean this enum holds is whether this decision is final or not.
    /// As an example, when the FSM first beings we start in
    /// [`StakeFiniteStateMachine::NotAStakeTransaction`] with the boolean set
    /// to [`false`] meaning that we can possibly advance the FSM.
    NotAStakeTransaction(bool),

    /// The resources have been withdrawn from an account and are currently in
    /// the worktop. This keeps track of how much were withdrawn and from
    /// which account. This does not keep track of
    /// any [`ResourceAddress`]es since only withdraws of XRD are permitted.
    /// Any other withdraw will put the FSM in a
    /// [`StakeFiniteStateMachine::NotAStakeTransaction(true)`] state.
    XrdWithdrawn {
    ...
    /// The XRD has been taken from the worktop and is now in a
    ...
    /// XRD has been staked to a validator and they have given us back some
    /// stake units of some amount.
    Staked {
        /// The address of the account that the XRD was withdrawn from.
        account_withdrawn_from: ComponentAddress,
        /// The amount of XRD that was withdrawn from the account.
        ...
        /// The address of the stake units that the validator has given back in
        /// return.
        stake_unit_address: ResourceAddress,
        /// The amount of stake units that the validator has given back in
        /// return.
    ...
    /// The liquid stake units are put in a bucket. To get to this state we
    /// first ensure that all of the XRD withdrawn has been used up.
    ...
    /// The stake units have been deposited into an account. It is checked that
    /// the account that they are deposited into is the same account that
...
impl StakeFiniteStateMachine {
    ...
    /// [`StakeFiniteStateMachine::NotAStakeTransaction(true)`] meaning that
    /// the FSM has concluded that this is not a stake transaction and that this
    /// conclusion is final.
    pub fn transition(
        &mut self,
        instruction: InstructionV1,
        ...
        match (&self, instruction) {
            // We can transition to [`XrdWithdrawn`] if we're in the initial
            // state or if we are in the [`DepositedIntoAccount`]
            // state and we get a CallMethod to account withdraw.
            (
            ...
            // The transition from XrdWithdrawn to XrdInBucket. This happens
            // when we're in the XrdWithdrawn state and we get a
            ...
            // The transition from XrdInBucket to Staked. This happens when we
            // do a CallMethod to a validator component
            (
                Self::XrdInBucket {
                    account_withdrawn_from,
                    withdrawn_amount,
            ...
            // The transition from Staked back to XrdInBucket - This happens
            // when we take from worktop by amount of XRD again
            // after staking (typically to stake again)
            (
                Self::Staked {
                    account_withdrawn_from,
                    withdrawn_amount,
            ...
            // The transition from Staked back to XrdInBucket - This happens
            // when we take from worktop of all of XRD again after
            // staking (typically to stake again)
            (
                Self::Staked {
                    account_withdrawn_from,
                    withdrawn_amount,
                ...
                *self = Self::XrdInBucket {
                    ...
                    withdrawn_amount: *withdrawn_amount,
                    xrd_used_up_so_far: *xrd_used_up_so_far,
                    amount_in_bucket: *withdrawn_amount - *xrd_used_up_so_far,
                }
            }
            // Transition from Staked to DepositedIntoAccount - This happens
            // when the deposit batch method is called on an account
            // while we're in the staked state. This transition fails
            // if the XRD used up so far doesn't equal that which has been
            // withdrawn.
            (
                Self::Staked {
                    account_withdrawn_from,
            ...
            // Transition from Staked to LiquidStakeUnitsInBucket - This happens
            // when we're in the Staked state and we get a
            // TakeAllFromWorktop instruction of the LSU's resource
            // address
            (
                Self::Staked {
                    account_withdrawn_from,
