...
impl FromStr for SerializableNonFungibleGlobalIdInternal {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let resource_address_string = s.split(':').next().ok_or(
            SerializableNonFungibleGlobalIdError::NoSeparatorFound(
                s.to_owned(),
            ),
        )?;

        let network_id =
            radix_engine_toolkit_core::utils::network_id_from_address_string(
        ...
        let non_fungible_global_id =
            NonFungibleGlobalId::try_from_canonical_string(&bech32_decoder, s)
                .map_err(|error| {
                    SerializableNonFungibleGlobalIdError::ParsingError(
                        debug_string(error),
                    )
                })?;
