...
use radix_engine::transaction::{
    ...
    TransactionReceipt, VersionedTransactionReceipt,
};
...
use transaction::validation::{
    NotarizedTransactionValidator, ValidationConfig,
};
            ...
            .map(|(manifest, receipt)| {
                let instructions =
                    to_serializable_instructions(&manifest.instructions, 0xf2)
                        .unwrap();
                let instructions =
                    SerializableInstructions::Parsed(instructions);
                let instructions =
                    InstructionsConvert::handle(InstructionsConvertInput {
                        instructions,
                ...
                let preview_receipt =
                    scrypto_encode(&VersionedTransactionReceipt::V1(receipt))
                        .unwrap();
