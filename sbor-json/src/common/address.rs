...
/// A representation of a serializable NodeId made up of a node id and a network. The network here
/// is:
/// * The network that will be used during serialization to Bech32m encode the node id.
/// * The network that was encountered while deserializing into this type and was used to bech32m
/// decode the address.
...
impl Display for SerializableNodeId {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let network_definition = crate::utils::network_definition_from_network_id(self.1);
...
impl FromStr for SerializableNodeId {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let network_id = crate::utils::network_id_from_address_string(s).ok_or(
            ParseSerializableNodeIdError::FailedToExtractNetworkIfFromAddress(s.to_owned()),
        )?;
