...
impl<'f> Function<'f> for SignedIntentHash {
    ...
    fn handle(
        ...
        let hash = radix_engine_toolkit::functions::signed_intent::hash(&signed_intent).map_err(
            |error| {
...
impl<'f> Function<'f> for SignedIntentCompile {
    ...
    fn handle(
        ...
        let signed_intent = signed_intent.to_native(*network_id)?;
        let compile = radix_engine_toolkit::functions::signed_intent::compile(&signed_intent)
            .map_err(|error| {
                InvocationHandlingError::EncodeError(
                    debug_string(error),
                    debug_string(signed_intent),
...
impl<'a> Function<'a> for SignedIntentDecompile {
    ...
    fn handle(
        ...
        let signed_intent = radix_engine_toolkit::functions::signed_intent::decompile(&**compiled)
            .map_err(|error| {
