...
impl<'f> Function<'f> for IntentHash {
    ...
    fn handle(
        ...
        let hash = radix_engine_toolkit::functions::intent::hash(&intent).map_err(|error| {
...
impl<'f> Function<'f> for IntentCompile {
    ...
    fn handle(
        ...
        let intent = intent.to_native(*network_id)?;
        let compile =
            radix_engine_toolkit::functions::intent::compile(&intent).map_err(|error| {
                InvocationHandlingError::EncodeError(debug_string(error), debug_string(intent))
            })?;
        Ok(compile.into())
    }
...
impl<'a> Function<'a> for IntentDecompile {
    ...
    fn handle(
        ...
        let intent =
            radix_engine_toolkit::functions::intent::decompile(&**compiled).map_err(|error| {
...
impl<'a> Function<'a> for IntentStaticallyValidate {
    ...
    fn handle(
        ...
        match radix_engine_toolkit::functions::intent::statically_validate(
            &intent,
            validation_config,
        ) {
