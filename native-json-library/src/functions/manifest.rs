...
impl<'f> Function<'f> for ManifestHash {
    ...
    fn handle(
        ...
        let hash = radix_engine_toolkit::functions::manifest::hash(&manifest).map_err(|error| {
...
impl<'f> Function<'f> for ManifestCompile {
    ...
    fn handle(
        ...
        let manifest = manifest.to_native(*network_id)?;
        let compile =
            radix_engine_toolkit::functions::manifest::compile(&manifest).map_err(|error| {
                InvocationHandlingError::EncodeError(debug_string(error), debug_string(manifest))
            })?;
        Ok(compile.into())
    }
...
impl<'a> Function<'a> for ManifestDecompile {
    ...
    fn handle(
        ...
        let manifest =
            radix_engine_toolkit::functions::manifest::decompile(&**compiled).map_err(|error| {
...
impl<'a> Function<'a> for ManifestStaticallyValidate {
    ...
    fn handle(
        ...
        match radix_engine_toolkit::functions::manifest::statically_validate(&manifest) {
