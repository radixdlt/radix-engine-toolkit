...
use radix_engine_toolkit::functions::execution::{TransactionType, FeeSummary};
...
impl<'f> Function<'f> for ExecutionAnalyze {
    ...
    fn handle(
        ...
        let execution_analysis =
            radix_engine_toolkit::functions::execution::analyze(&instructions, &receipt).map_err(
                |error| InvocationHandlingError::InstructionVisitorError(debug_string(error)),
            )?;
