...
const main = async (): Promise<void> => {
	...
	let compiledSignedTransactionIntent: string = (transactionService
		.compileSignedTransactionIntent(signedTransactionIntent) as CompileSignedTransactionIntentResponse)
		.compiled_signed_intent;
	let notarySignature: Uint8Array = keyPair.sign(byteArrayFromHex(compiledSignedTransactionIntent));
	
	let compiledTransaction: string = (transactionService.compileNotarizedTransactionIntent({
