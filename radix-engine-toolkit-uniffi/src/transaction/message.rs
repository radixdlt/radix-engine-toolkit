...
impl From<NativePlaintextMessage> for PlainTextMessage {
    fn from(NativePlaintextMessage { message, mime_type }: NativePlaintextMessage) -> Self {
        Self {
            message: message.into(),
            mime_type,
        }
...
impl TryFrom<Message> for NativeMessage {
    ...
    fn try_from(value: Message) -> Result<Self> {
        match value {
            ...
            Message::Encrypted { value } => value.try_into().map(NativeMessage::Encrypted),
            Message::PlainText { value } => Ok(NativeMessage::Plaintext(value.into())),
        }
    }
}
