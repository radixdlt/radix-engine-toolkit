...
impl ManifestBuilderValue {
    pub fn to_native(&self, name_record: &NameRecord) -> Result<NativeManifestValue> {
            ...
            Self::MapValue {
                ...
                entries: entries
                    ...
                    .map(|ManifestBuilderMapEntry { key, value }| {
                        key.to_native(name_record)
                            .and_then(|key| value.to_native(name_record).map(|value| (key, value)))
                    })
                    .collect::<Result<_>>()?,
            },
            Self::AddressValue { value } => NativeManifestValue::Custom {
                value: NativeManifestCustomValue::Address(value.to_native(name_record)?),
            },
            Self::BucketValue { value } => NativeManifestValue::Custom {
                value: NativeManifestCustomValue::Bucket(value.to_native(name_record)?),
            },
            Self::ProofValue { value } => NativeManifestValue::Custom {
                value: NativeManifestCustomValue::Proof(value.to_native(name_record)?),
            },
            Self::AddressReservationValue { value } => NativeManifestValue::Custom {
                value: NativeManifestCustomValue::AddressReservation(value.to_native(name_record)?),
            },
